...
matrixByRows(L, NumCols, MatrixByRows):-
            ...
            append(MatrixByRows, L). % fica totes les variables de L dins la matriu (llsita de llistes)
...
declareConstraints(Mrows, Mtransp, Rsums, Csums, L):-
                   L ins 0..1,
									 write("Provant suma files:"),nl,
                   checkSum(Mrows, Rsums),
									 write("Provant suma columnes:"),nl,
                   checkSum(Mtransp, Csums),
                   label(L).

checkSum([], []):- !.
checkSum([R|Matrix], [S|Sum]):- sum(R, #=, S),write(R),write("   ->     "),write(S),nl,nl,nl, checkSum(Matrix, Sum).
% sum(+Vars, +Rel, ?Expr)
