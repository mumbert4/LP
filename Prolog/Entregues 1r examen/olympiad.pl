...
noSuperarMembres:-numMembersTeam(X) ,findall(chosen(S), student(S), Lits), exactly(X,Lits),fail.
noSuperarMembres.

totsTopics:- expertise(_,E),findall(chosen(S),expertise(S,E), Lits), atMost(1,Lits),fail.
totsTopics.

amics:- student(S1), student(S2), not(friends(S1,S2)), writeClause([-chosen(S1), -chosen(S2)]),fail. % averiguar exactament com funciona lo de writeClause
amics.
