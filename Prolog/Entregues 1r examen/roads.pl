...
% UNA BENZINERA HA DE CONNECTAR DUES CIUTATS, UNA BENZINERA HA D'ESTAR A UNA CIUTAT, A L'ALTRE O A LES DUES
...
writeClauses:-
    % noSuperarMaxGasolineras,
    cadaCarreteraUnaGasolinera, true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.


cadaCarreteraUnaGasolinera:- city(C1), city(C2), road(C1,C2),findall(install(C), member(C,[C1,C2]),Lits), atLeast(1,Lits), fail.
cadaCarreteraUnaGasolinera.

noSuperarMaxGasolineras:- maxStations(X),findall(install(C), city(C), Lits), atMost(X,Lits),fail.
noSuperarMaxGasolineras.
