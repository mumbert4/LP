...
writeClauses:-
  sonTodosAmigos,
  exactamenteUnTemaEnElEquipo,
  exactamenteNumMaxEquipo,
  true, !.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactamenteNumMaxEquipo:- numMembersTeam(X), findall(chosen(S), student(S), Lits), exactly(X, Lits), fail.
exactamenteNumMaxEquipo.

exactamenteUnTemaEnElEquipo:- expertise(_, E), findall(chosen(S1), expertise(S1, E), Lits), atMost(1, Lits), fail.
exactamenteUnTemaEnElEquipo.

sonTodosAmigos:- student(S1), student(S2), not(friends(S1, S2)), writeClause([-chosen(S1), -chosen(S2)]), fail.
sonTodosAmigos.
...
%displaySol(M):- student(S), member(chosen(S), M), write('chosen('), write(S), write(').'), nl, fail.
displaySol(_):- nl.
