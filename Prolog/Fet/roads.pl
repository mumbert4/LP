...
writeClauses:-
  paraCadaCarreteraUnagasolinera,
  noSuperarMaxGasolineras,
  true, !.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

paraCadaCarreteraUnagasolinera:- city(C1), city(C2), road(C1, C2), findall(install(C), member(C, [C1, C2]), Lits), atLeast(1, Lits), fail.
paraCadaCarreteraUnagasolinera.
...
displaySol(M):- nl, city(C), member(install(C), M), write('install('), write(C), write(').'), nl, fail.
displaySol(_):- nl.
