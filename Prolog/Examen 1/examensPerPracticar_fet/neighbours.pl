...
writeClauses:-
    exactamenteK,
    allDiferentNeighbourhood,
    cadaHabitanteRepresentado,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactamenteK:- findall(chosen(N), neighbor(N), Lits), numRepresentatives(X), exactly(X,Lits),false.
exactamenteK.

allDiferentNeighbourhood:- neighborNeighborhood(_,NH) ,findall(chosen(N), neighborNeighborhood(N,NH), Lits), atMost(1,Lits),false.
allDiferentNeighbourhood.

cadaHabitanteRepresentado:- neighbor(N), neighborRepresentedBy(N1,L), findall(chosen(N2), member(N2,L), Lits), atLeast(1,Lits).
cadaHabitanteRepresentado. 
