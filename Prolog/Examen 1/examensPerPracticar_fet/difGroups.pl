...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    student(S1), student(S2),S1< S2 ,lastYearTeam(L),member(S1,L), member(S2,L) , true. % COMPLETE THIS!

%%%%%%  SAT Variables:
satVariable( st(S,T) ):- student(S), team(T). % student S goes to team T
satVariable(in(T)):-team(T).
writeClauses:-
    allStudents1Team,
    % allTeam1Student,
    exactMembers,
...
allStudents1Team:- student(S), findall(st(S,T), team(T), Lits), exactly(1,Lits),fail.
allStudents1Team.

allTeam1Student:- team(T), findall(st(S,T), student(S), Lits), exactly(1,Lits),fail.
allTeam1Student.

exactMembers:-team(T), findall(st(S,T), student(S), Lits), teamSize(X),exactly(X,Lits),fail.
exactMembers.

sameTeam:- student(S1), student(S2), incompatible(S1,S2),team(T), writeClause([-st(S1,T), -st(S2,T), in(T)]),fail.
sameTeam.

maxTeams:- findall(in(T), team(T), Lits), atMost(1,Lits),fail.
maxTeams.
%%%%%%%%%%%%%%%%%%%%%%%
