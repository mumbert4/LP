...
exactRepresentants:- findall(chosen(N), neighbor(N), Lits), numRepresentatives(X),exactly(X,Lits),fail.
exactRepresentants.

everyHood:- neighborNeighborhood(_,NH), findall(chosen(N), neighborNeighborhood(N,NH),Lits), exactly(1,Lits),fail.
everyHood.

allRepresented:- neighbor(N), neighborRepresentedBy(N,L), findall(chosen(N2), member(N2,L), Lits), atLeast(2,Lits),fail.
allRepresented.
