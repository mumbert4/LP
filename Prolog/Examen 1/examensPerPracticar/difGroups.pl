...
exactSize:-team(T), findall(st(S,T), student(S), Lits), teamSize(X),exactly(X,Lits), fail.
exactSize.

oneStudentALlTeams:- student(S), findall(st(S,T), team(T), Lits), exactly(1,Lits),fail.
oneStudentALlTeams.

compatible:- student(S1),student(S2),student(S3), incompatible(S1,S2), incompatible(S1,S3), S3 \= S2, team(T),writeClause([-st(S1,T),-st(S2,T),-st(S3,T)]), fail.
compatible.
