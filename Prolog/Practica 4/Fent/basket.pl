...
eachTeamVSeachTeam:- difTeams(S,T),
    findall(match(S,T,R), round(R), LitsA),
    findall(match(T,S,R), round(R), LitsB), append(LitsA,LitsB,Lits), exactly(1,Lits),fail.
eachTeamVSeachTeam.
...
homeGame:- team(T), difTeams(T,S), round(R), writeClause([-match(T,S,R), home(T,R)]),fail.
homeGame.

awayGame:- team(T), difTeams(T,S), round(R), writeClause([-match(S,T,R), -home(T,R)]),fail.
awayGame.

doubleHome:- team(T), round(R1), round(R2), R2 is R1 - 1, writeClause([-home(T,R1), -home(T,R2), double(T,R1)]),fail.
doubleHome.

doubleAway:- team(T), round(R1), round(R2), R2 is R1 - 1, writeClause([home(T,R1), home(T,R2), double(T,R1)]),fail.
doubleAway.

noHomeGame:- team(T), away(T,R), writeClause([-home(T,R)]),fail.
noHomeGame.

noTripleHome:- team(T), round(R1), round(R2), round(R3), R2 is R1 + 1, R3 is R2 + 1, writeClause([-home(T,R1), -home(T,R2), - home(T, R3)]),fail.
noTripleHome.

noTripleAway:- team(T), round(R1), round(R2), round(R3), R2 is R1 + 1, R3 is R2 + 1, writeClause([home(T,R1), home(T,R2), home(T, R3)]),fail.
noTripleAway.

noDoubleOnRounds:- noDoubles(L), member(X,L), team(T), writeClause([-double(T,X)]),fail.
noDoubleOnRounds.
