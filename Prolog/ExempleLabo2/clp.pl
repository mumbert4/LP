...
matrixByRows(NumCol, Vars, Matrix):-
  length(Matrix,NumCol),
  maplist(checkLength(NumCol), Matrix),
  append(Matrix, Vars).

checkLength(NumCols, X):- length(X,NumCols).

constraintsFromSubLists(Matrix):-
  maplist(all_different(), Matrix).
