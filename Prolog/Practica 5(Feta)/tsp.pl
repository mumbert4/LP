...
distances(_,[],[]).
distances(City1,[City2|RestoCiudades],[Distancia| RestoDist]):-
  distance(City1,City2,Distancia),
  distances(City1,RestoCiudades,RestoDist).


minList(Min, [Min]).
minList(Min, [H|T]) :- minList(PMin, T), Min is min(H, PMin).

myselect(CurrentCity, City, Cities, RemainingCities):-
  distances(CurrentCity,Cities,Dist),
  minList(D,Dist),
  distance(CurrentCity,City,D),
