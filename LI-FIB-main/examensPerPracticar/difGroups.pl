...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    ...
    lastYearTeam(L), member(S1, L), member(S2, L),
    true. % COMPLETE THIS!
...
allTeamsSameSize:- teamSize(X), team(T), findall(st(S, T), student(S), Lits), exactly(X, Lits), fail.
allTeamsSameSize.

oneStudentOneTeam:- student(S), findall(st(S, T), team(T), Lits), exactly(1, Lits), fail.
oneStudentOneTeam.

atMost1FromLastYear:- team(T), incompatible(S, S1), incompatible(S, S2), S1 \= S2, writeClause([-st(S, T), -st(S1, T), -st(S2, T)]), fail.
atMost1FromLastYear.
