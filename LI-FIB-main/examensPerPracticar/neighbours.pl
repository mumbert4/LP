...
writeClauses:-
  exactamenteNRepresentantes,
  comoMuchoUnRepresentanteDeUnBarrio,
  existeComoMinimoUnRepresentante,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactamenteNRepresentantes:- numRepresentatives(X), findall(chosen(N), neighbor(N), Lits), exactly(X, Lits), fail.
exactamenteNRepresentantes.

comoMuchoUnRepresentanteDeUnBarrio:- neighborNeighborhood(_, B), findall(chosen(N), neighborNeighborhood(N, B), Lits), atMost(1, Lits), fail.
comoMuchoUnRepresentanteDeUnBarrio.

existeComoMinimoUnRepresentante:- neighborRepresentedBy(_, L), findall(chosen(N), member(N, L), Lits), atLeast(1, Lits), fail.
existeComoMinimoUnRepresentante.
